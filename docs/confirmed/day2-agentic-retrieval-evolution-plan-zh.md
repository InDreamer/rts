...
children: []
summary: Day2 evolution plan for controlled agentic retrieval, reranking, MCP expansion, and LLM-assisted analysis in RTS
read_when:
  - 需要规划 Day1 之后 RTS 查询/索引/LLM 服务的升级方向
  - 需要判断哪些能力应该 Day2 做而不是 Day1 做
  - 需要对齐 agentic retrieval、rerank、negative retrieval、向量检索、MCP 扩展和 impact/test planning 的边界
skip_when:
  - 只需要第一版可实现方案
