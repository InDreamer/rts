...
children: []
summary: final RTS service plan for LLM-agent-centered consumption, controlled harness, governed truth access, context management, memory boundary, API/MCP support, and implementation roadmap
read_when:
  - 需要理解 RTS 最终如何服务 LLM agent
  - 需要设计 LLM harness 在 RTS 中的核心职责
  - 需要定义 RTS 如何约束、支撑和审计 LLM agent
  - 需要规划 API/MCP/Q&A/pipeline 统一 truth service
  - 需要把 Day1/Day2 演进对齐到最终服务形态
skip_when:
  - 只需要当前 Day1 代码行为
